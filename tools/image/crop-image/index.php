    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form id="tool-form" class="animate-fade-in" method="POST" enctype="multipart/form-data">
            ...
            <div class="glass-card rounded-3xl p-8 mb-10">
                <h3 class="text-white font-bold mb-6 flex items-center gap-2"><i data-lucide="sliders" class="w-5 h-5 text-indigo-400"></i> Crop Area</h3>
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div><label class="block text-gray-400 text-sm mb-2">X Position (px)</label><input type="number" name="crop_x" value="0" min="0" class="w-full px-4 py-3 rounded-xl bg-gray-900 border border-gray-800 text-white"></div>
                    <div><label class="block text-gray-400 text-sm mb-2">Y Position (px)</label><input type="number" name="crop_y" value="0" min="0" class="w-full px-4 py-3 rounded-xl bg-gray-900 border border-gray-800 text-white"></div>
                </div>
                <div class="grid grid-cols-2 gap-6">
                    <div><label class="block text-gray-400 text-sm mb-2">Crop Width (px)</label><input type="number" name="crop_w" value="500" min="1" class="w-full px-4 py-3 rounded-xl bg-gray-900 border border-gray-800 text-white"></div>
                    <div><label class="block text-gray-400 text-sm mb-2">Crop Height (px)</label><input type="number" name="crop_h" value="500" min="1" class="w-full px-4 py-3 rounded-xl bg-gray-900 border border-gray-800 text-white"></div>
                </div>
                <p class="text-gray-500 text-xs mt-4">Tip: X and Y define the top-left corner of the crop area. Width and Height define the crop region size.</p>
            </div>

            <button type="submit" class="w-full py-5 rounded-3xl bg-indigo-600 text-white font-extrabold text-lg hover:bg-indigo-500 shadow-2xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-3">Crop Image <i data-lucide="arrow-right" class="w-6 h-6"></i></button>
        </form>
