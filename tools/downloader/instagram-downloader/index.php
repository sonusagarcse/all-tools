    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <div class="flex gap-3">
                <div class="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 flex items-center gap-2 text-purple-400">
                    <i data-lucide="instagram" class="w-5 h-5"></i>
                    <span class="text-sm font-medium">Reels & Posts</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form id="url-form" class="animate-fade-in mb-12">
            ...
            <div class="relative group">
                ...
                <input type="url" name="url" required class="block w-full pl-14 pr-32 py-5 rounded-3xl bg-gray-900 border border-gray-800 text-lg text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all shadow-2xl" placeholder="Paste Instagram link here...">
                <button type="submit" class="absolute right-3 top-2.5 px-6 py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold hover:from-purple-500 hover:to-pink-500 transition-all flex items-center gap-2">
        ...
        <div id="progress-overlay" class="hidden animate-fade-in py-20 text-center">
            <div class="relative w-24 h-24 mx-auto mb-8"><div class="absolute inset-0 border-4 border-gray-800 rounded-full"></div><div class="absolute inset-0 border-4 border-purple-500 rounded-full border-t-transparent animate-spin"></div></div>
            <h3 class="text-2xl font-bold text-white mb-2">Analyzing URL</h3>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <form id="download-form" action="process.php" method="POST">
                        ...
                        <div class="space-y-4 mb-8">
                            ...
                            <div id="format-list" class="grid grid-cols-1 sm:grid-cols-2 gap-3"></div>
                        </div>
                        <button type="submit" class="w-full py-4 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-extrabold text-lg hover:from-purple-500 hover:to-pink-500 shadow-xl transition-all flex items-center justify-center gap-2">
        ...
        <div id="final-download" class="hidden animate-fade-in py-12">
            <div class="glass-card rounded-[40px] p-8 md:p-12 text-center border-purple-500/20 shadow-purple-500/10">
                <div class="w-20 h-20 bg-green-500/10 rounded-3xl flex items-center justify-center text-green-500 mx-auto mb-8"><i data-lucide="check-circle" class="w-12 h-12"></i></div>
                <h2 class="text-3xl font-bold text-white mb-4">File is Ready!</h2>
                <div class="flex gap-4 justify-center mt-10">
                    <a id="final-btn" href="#" class="px-10 py-5 rounded-3xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-extrabold text-lg hover:from-purple-500 hover:to-pink-500 transition-all shadow-xl flex items-center justify-center gap-2"><i data-lucide="download" class="w-6 h-6"></i> Download File</a>
                </div>
            </div>
        </div>
    </div>
        ...
        try {
            ...
            if (result.success) {
                ...
                result.formats.forEach(f => {
                    ...
                    div.innerHTML = `<input type="radio" name="format_id" value="${f.format_id}" checked class="peer sr-only"><div class="px-4 py-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-400 text-sm flex justify-between items-center peer-checked:border-purple-500 peer-checked:bg-purple-500/10 peer-checked:text-white transition-all"><span>${f.quality} (${f.ext})</span><span class="opacity-50">${f.filesize_str}</span></div>`;
