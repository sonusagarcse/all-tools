...
if (strpos($url, 'tiktok.com') === false && strpos($url, 'vm.tiktok.com') === false) { json_error('Please enter a valid TikTok URL.'); }
...
if ($action === 'fetch_info') {
    ...
    if (!$output) { json_error('Failed to fetch video info. The video may be private or region-locked.'); }
    ...
    json_response(['title' => $data['title'] ?? 'TikTok Video', 'thumbnail' => $data['thumbnail'] ?? '', 'duration_string' => isset($data['duration']) ? gmdate("H:i:s", $data['duration']) : '0:00', 'uploader' => $data['uploader'] ?? $data['creator'] ?? 'Unknown', 'webpage_url' => $data['webpage_url'] ?? $url, 'formats' => array_slice($formats, 0, 8)]);

} elseif ($action === 'download') {
    $formatId = $_POST['format_id'] ?? 'best';
    if (!rate_limit_check()) { json_error('Rate limit exceeded.'); }
    $tempPrefix = 'tiktok_' . uniqid();
