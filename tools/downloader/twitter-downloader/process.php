...
if (strpos($url, 'twitter.com') === false && strpos($url, 'x.com') === false && strpos($url, 't.co') === false) { json_error('Please enter a valid Twitter/X URL.'); }
...
if ($action === 'fetch_info') {
    ...
    if (!$output) { json_error('Failed to fetch video. The tweet may not contain a video.'); }
    ...
    json_response(['title' => $data['title'] ?? 'Twitter Video', 'thumbnail' => $data['thumbnail'] ?? '', 'duration_string' => isset($data['duration']) ? gmdate("H:i:s", $data['duration']) : '0:00', 'uploader' => $data['uploader'] ?? $data['uploader_id'] ?? 'Unknown', 'webpage_url' => $data['webpage_url'] ?? $url, 'formats' => array_slice($formats, 0, 8)]);

} elseif ($action === 'download') {
    $formatId = $_POST['format_id'] ?? 'best';
    if (!rate_limit_check()) { json_error('Rate limit exceeded.'); }
    $tempPrefix = 'tweet_' . uniqid();
