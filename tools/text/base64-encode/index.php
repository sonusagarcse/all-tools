        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <div class="flex gap-3">
                <div class="px-4 py-2 rounded-xl bg-indigo-600/10 border border-indigo-600/20 flex items-center gap-2 text-indigo-400 shadow-lg shadow-indigo-600/5 transition-all">
                    ...
                    <span class="text-sm font-medium">Instant Encoding</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-6">
            <div class="relative group">
                <label for="text-input" class="block text-white font-bold mb-2">Input Text</label>
                <textarea id="text-input" class="w-full h-[200px] p-6 rounded-3xl bg-gray-900 border border-gray-800 text-white text-lg placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all shadow-xl resize-none" placeholder="Type or paste text to encode..."></textarea>
            </div>
            ...
            <div class="relative group">
                <label for="text-output" class="block text-white font-bold mb-2">Base64 Encoded Result</label>
                <textarea id="text-output" class="w-full h-[200px] p-6 rounded-3xl bg-gray-900 border border-gray-800 text-indigo-400 text-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-xl resize-none" readonly placeholder="Encoded output will appear here..."></textarea>
                ...
                <button onclick="copyToClipboard('text-output')" class="absolute bottom-6 right-6 p-3 rounded-xl bg-indigo-600 text-white hover:bg-indigo-500 transition-all flex items-center gap-2 text-sm font-bold shadow-lg" title="Copy result">
                    <i data-lucide="copy" class="w-5 h-5"></i> Copy Base64
                </button>
            </div>
        </div>
...
<script>
    ...
    inputArea.addEventListener('input', () => {
        try {
            ...
            // Handle utf-8 string encoding to base64
            outputArea.value = text ? btoa(unescape(encodeURIComponent(text))) : '';
        } catch(e) {
            outputArea.value = 'Error encoding text';
        }
    });
</script>
