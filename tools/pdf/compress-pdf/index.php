    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <div class="px-4 py-2 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center gap-2 text-orange-400">
                ...
                <span class="text-sm font-medium">Ghostscript Optimization</span>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form id="tool-form" class="animate-fade-in" method="POST" enctype="multipart/form-data">
            ...
            <div class="upload-zone rounded-[40px] p-12 text-center cursor-pointer group mb-10">
                ...
                <div class="w-20 h-20 bg-indigo-600/10 rounded-full flex items-center justify-center text-indigo-400 mx-auto mb-8 group-hover:bg-indigo-600 group-hover:text-white transition-all"><i data-lucide="file-down" class="w-10 h-10"></i></div>
                <h2 class="text-2xl font-bold text-white mb-3">Choose PDF File</h2>
                <p class="text-gray-400 mb-8">Reduce file size while testing quality</p>
                <div class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-indigo-600 text-white font-bold hover:bg-indigo-500 shadow-lg">Browse File</div>
            ...
            <div class="glass-card rounded-3xl p-8 mb-10">
                <h3 class="text-white font-bold mb-6 flex items-center gap-2"><i data-lucide="settings" class="w-5 h-5 text-indigo-400"></i> Optimization Level</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label class="cursor-pointer"><input type="radio" name="level" value="screen" checked class="peer sr-only"><div class="px-4 py-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-400 text-center peer-checked:border-indigo-500 peer-checked:bg-indigo-500/10 peer-checked:text-white">Minimum (72 DPI)</div></label>
                    <label class="cursor-pointer"><input type="radio" name="level" value="ebook" class="peer sr-only"><div class="px-4 py-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-400 text-center peer-checked:border-indigo-500 peer-checked:bg-indigo-500/10 peer-checked:text-white">Medium (150 DPI)</div></label>
                    <label class="cursor-pointer"><input type="radio" name="level" value="printer" class="peer sr-only"><div class="px-4 py-3 rounded-xl border border-gray-800 bg-gray-900 text-gray-400 text-center peer-checked:border-indigo-500 peer-checked:bg-indigo-500/10 peer-checked:text-white">Maximum (300 DPI)</div></label>
                </div>
            </div>

            <button type="submit" class="w-full py-5 rounded-3xl bg-indigo-600 text-white font-extrabold text-lg hover:bg-indigo-500 shadow-2xl flex items-center justify-center gap-3">Compress PDF <i data-lucide="zap" class="w-6 h-6"></i></button>
        </form>

        <div id="progress-overlay" class="hidden animate-fade-in py-20 text-center"><div class="w-24 h-24 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin mx-auto mb-8"></div><h3 class="text-2xl font-bold text-white mb-2">Compressing Document</h3><p class="text-gray-400">Executing Ghostscript engine...</p></div>

        <div id="result-section" class="hidden animate-fade-in py-12"><div class="glass-card rounded-[40px] p-8 md:p-12 text-center border-indigo-500/20 shadow-indigo-500/10"><div class="w-20 h-20 bg-green-500/10 rounded-3xl flex items-center justify-center text-green-500 mx-auto mb-8"><i data-lucide="check-circle" class="w-12 h-12"></i></div><h2 class="text-3xl font-bold text-white mb-4">Compression Success!</h2><div class="flex gap-4 justify-center mt-10"><a id="download-btn" href="#" class="px-10 py-5 rounded-3xl bg-indigo-600 text-white font-extrabold text-lg flex items-center gap-2"><i data-lucide="download" class="w-6 h-6"></i> Download PDF</a></div></div></div>
    </div>
</section>
