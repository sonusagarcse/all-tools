...
<section class="pt-12 pb-8 bg-gray-950">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <div class="px-4 py-2 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center gap-2 text-cyan-400">
                <i data-lucide="scissors" class="w-5 h-5"></i>
                <span class="text-sm font-medium">Extract specific pages</span>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form id="tool-form" class="animate-fade-in" method="POST" enctype="multipart/form-data">
            ...
            <div class="upload-zone rounded-[40px] p-12 text-center cursor-pointer group mb-10">
                ...
                <div class="w-20 h-20 bg-indigo-600/10 rounded-full flex items-center justify-center text-indigo-400 mx-auto mb-8 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300"><i data-lucide="scissors" class="w-10 h-10"></i></div>
            ...
            <div class="glass-card rounded-3xl p-8 mb-10">
                <h3 class="text-white font-bold mb-6 flex items-center gap-2"><i data-lucide="settings" class="w-5 h-5 text-indigo-400"></i> Page Selection</h3>
                <div>
                    <label class="block text-gray-400 text-sm font-medium mb-3">Pages to Extract</label>
                    <input type="text" name="pages" value="1-3" placeholder="e.g. 1-3, 5, 7-10" class="w-full px-4 py-3 rounded-xl border border-gray-800 bg-gray-900 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <p class="text-gray-500 text-xs mt-2">Use commas for individual pages and hyphens for ranges. Example: 1-3, 5, 7-10</p>
                </div>
            </div>

            <button type="submit" class="w-full py-5 rounded-3xl bg-indigo-600 text-white font-extrabold text-lg hover:bg-indigo-500 shadow-2xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-3">Split PDF <i data-lucide="arrow-right" class="w-6 h-6"></i></button>
        </form>

        <div id="progress-overlay" class="hidden animate-fade-in py-20 text-center"><div class="w-24 h-24 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin mx-auto mb-8"></div><h3 class="text-2xl font-bold text-white mb-2">Splitting PDF</h3><p class="text-gray-400">Extracting selected pages...</p></div>

        <div id="result-section" class="hidden animate-fade-in py-12"><div class="glass-card rounded-[40px] p-8 md:p-12 text-center border-indigo-500/20 shadow-indigo-500/10"><div class="w-20 h-20 bg-green-500/10 rounded-3xl flex items-center justify-center text-green-500 mx-auto mb-8"><i data-lucide="check-circle" class="w-12 h-12"></i></div><h2 class="text-3xl font-bold text-white mb-4">PDF Split Successfully!</h2><div class="flex gap-4 justify-center mt-10"><a id="download-btn" href="#" class="px-10 py-5 rounded-3xl bg-indigo-600 text-white font-extrabold text-lg flex items-center gap-2"><i data-lucide="download" class="w-6 h-6"></i> Download PDF</a></div></div></div>
    </div>
</section>
